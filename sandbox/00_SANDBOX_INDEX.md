...
*The test bed for the Orchestration OS: where roles (resumes, ceremonies, contracts) are put through the ringer before they ship. Every role is run in describe mode against a task built to tempt its failure mode, then graded by a separate skeptic. Run never equals grade.*

← [[00_MOC|Orchestration OS]]

## Contents

- [[sandbox/role-conformance-harness|role-conformance-harness]] - the two-step method (RUN then EVALUATE) that validates a role stays in-role: the rubric, the read-only-at-the-tool-layer caveat, the hybrid-role architecture probe, and how to use the loop as a prompt foundry.
